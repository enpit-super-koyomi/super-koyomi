...
export function ConfirmationPopup({
  ...
  const [startTime, setStartTime] = useState(
    initialPeriod.start.toTimeString().slice(0, 5),
  );
  const [endTime, setEndTime] = useState(
    initialPeriod.end.toTimeString().slice(0, 5),
  );
  ...
  const handleChangeTime =
    (changed: keyof Period) => (e: ChangeEvent<HTMLInputElement>) => {
  ...
  const isOutOfRange = (period: Period) =>
    range ? period.start < range.start || period.end > range.end : true;

  const getFixedLengthPeriod = useCallback(
    (basePeriod: Period, period: Partial<Period>): Period => {
      if (period.start) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={isOutOfRange(period)}>
            Confirm
          </Button>
        </DialogFooter>
      </DialogContent>
